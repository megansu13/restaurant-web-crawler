...
while num_pages < 5:
    ...
    while count < 10:
        ...
        pages = page_soup.find("div", {"class": "pagination-links__09f24__bmFj8 border-color--default__09f24__NPAKY nowrap__09f24__lBkC2"})
        for page in pages.find_all('a', href=True):
            next_page = page['href']
        #next_page = pages.find("div", {"class": "navigation-button-container__09f24__SvcBh border-color--default__09f24__NPAKY"}).a.get('href')
        source = url.urlopen(next_page)            
        page_soup = bs.BeautifulSoup(source, 'html.parser')
    num_pages += 1
